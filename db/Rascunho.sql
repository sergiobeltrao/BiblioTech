...
('Ray Bradbury', 'Estados Unidos - Americana', 'MASC');
...
VALUES (1, 16), (2, 3), (3, 6), (4, 7), (5, 8), (6, 9), (7, 10), (8, 11), (9, 12), (10, 13), (11, 14), (12, 15);  
...
SELECT A.AVA_ID_USUARIO, COUNT(*) AS QUANTIDADE_DE_AVALIACOES
FROM AVALIACAO A
JOIN LIVRO L ON A.AVA_FK_LIVRO = L.ID_LIVRO
GROUP BY A.AVA_ID_USUARIO
ORDER BY QUANTIDADE_DE_AVALIACOES DESC;

SELECT U.USE_NICK, COUNT(*) AS QUANTIDADE_DE_AVALIACOES
FROM AVALIACAO A
JOIN LIVRO L ON A.AVA_FK_LIVRO = L.ID_LIVRO
...
SELECT U.USE_NICK, A.AVA_USUARIO, A.AVA_COMENTARIO
FROM USUARIO U
JOIN AVALIACAO A ON U.USE_ID = A.AVA_ID_USUARIO
ORDER BY A.AVA_DATA_AVALIACAO DESC;

SELECT L.Liv_NOME_LIVRO, U.USE_NICK, A.AVA_USUARIO, A.AVA_COMENTARIO 
FROM USUARIO U
JOIN AVALIACAO A ON U.USE_ID = A.AVA_ID_USUARIO
JOIN LIVRO L ON A.AVA_FK_LIVRO = L.ID_LIVRO
ORDER BY A.AVA_DATA_AVALIACAO DESC;

SELECT LIVRO.ID_LIVRO, LIVRO.LIV_NOME_LIVRO, AVALIACAO.AVA_USUARIO, AVALIACAO.AVA_COMENTARIO 
FROM AVALIACAO INNER JOIN LIVRO ON AVALIACAO.AVA_FK_LIVRO = LIVRO.ID_LIVRO;
...
DELETE FROM AVALIACAO WHERE AVA_ID_USUARIO = 1 AND AVA_USUARIO = 10.0 AND AVA_COMENTARIO = 'Great';
...
INNER JOIN
    USUARIO ON AVALIACAO.AVA_ID_USUARIO = USUARIO.USE_ID
WHERE
    LIVRO.LIV_NOME_LIVRO = 'O Segredo das Estrelas';
...
SELECT END_ID FROM USUARIO U
LEFT JOIN ENDERECO_USUARIO EU ON U.USE_ID = EU.ENDERECO_USER
LEFT JOIN ENDERECO E ON EU.ENDERECO_CHAVE = E.END_ID
LEFT JOIN TELEFONE_USUARIO TU ON U.USE_ID = TU.TELEFONE_USER
LEFT JOIN TELEFONE T ON TU.TELEFONE_FONE = T.TEL_ID
LEFT JOIN FAVORITO F ON U.USE_ID = F.FAV_ID_USUARIO WHERE USE_ID = 1;

SELECT E.END_ID FROM USUARIO U
                LEFT JOIN ENDERECO_USUARIO EU ON U.USE_ID = EU.ENDERECO_USER
                LEFT JOIN ENDERECO E ON EU.ENDERECO_CHAVE = E.END_ID
                WHERE U.USE_ID = 8;
                
SELECT F.FAV_ID_USUARIO
FROM USUARIO U
LEFT JOIN FAVORITO F ON U.USE_ID = F.FAV_ID_USUARIO
WHERE U.USE_ID = 1;
